  ...
  template: `
    <div class="flex flex-col gap-4 p-4 flex-1 min-h-0">
      ...
      @if (items().length === 0) {
        ...
        <app-paginator [currentPage]="currentPage()" [pageCount]="pageCount()" ariaLabel="Items pagination" (previous)="prev()" (next)="next()" [pageSize]="limit()" (pageSizeChange)="setPageSize($event)" />
      }
    </div>
  `,
})
