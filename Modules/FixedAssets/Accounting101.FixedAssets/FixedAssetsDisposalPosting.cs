...
/// <para>The Accumulated Depreciation line debits only <c>currentAccumulated</c>, dimensioned
/// <c>{Asset=assetId}</c> so the GL directly reflects which asset's accumulated depreciation is being
/// cleared: the catch-up's own contribution to accumulated depreciation and its immediate clearing on
/// disposal net to zero, leaving the depreciation expense in P&amp;L and only the pre-existing
/// accumulated cleared.</para></summary>
