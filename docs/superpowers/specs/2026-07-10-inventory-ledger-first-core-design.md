...
7. **Void auto-rolls-back via the ledger** (the FA payoff). Reversing the movement's GL entry drops it from the value fold and gates it out of the quantity projection, so the manual valuation-restore in `VoidAsync` (subtracting `SignedQuantityEffect`/`SignedValueEffect` via `SetValuationAsync`) is **deleted**, along with those effect properties and `SetValuationAsync` itself. LIFO enforcement stays.
...
- **`ResultingOnHand`/`ResultingTotalValue` removed** from `StockMovementBody` and `StockMovement`; **`SignedQuantityEffect`/`SignedValueEffect` deleted**. `Quantity` (authoritative), `AppliedUnitCost`/`ExtendedCost` (frozen evidentiary), `Type`, `EffectiveDate`, `Memo` stay. `StockMovementView` loses the two `Resulting*` fields (minor movement-screen touch if displayed — confirm in planning).
