...
public sealed class PeriodCloseTests(MongoFixture fixture) : IClassFixture<MongoFixture>
{
    ...
    {
        (LedgerService service, _, _, MongoCheckpointStore checkpoints) = NewLedger();
        ...
        IReadOnlyDictionary<Guid, decimal> snapshot = await service.CloseAsync(client, new DateOnly(2026, 3, 31), Guid.NewGuid());
    ...
    {
        (LedgerService service, _, _, _) = NewLedger();
        Guid client = Guid.NewGuid();
        Guid a = Guid.NewGuid();
        Guid b = Guid.NewGuid();

        await service.CloseAsync(client, new DateOnly(2026, 3, 31), Guid.NewGuid());

        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            service.PostAsync(Entry(client, 1, new DateOnly(2026, 3, 15), a, b, 10m))); // back-dated into closed period

        await service.PostAsync(Entry(client, 2, new DateOnly(2026, 4, 1), a, b, 10m)); // open period — fine
    }
    ...
    public async Task A_closed_period_entry_cannot_be_voided()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() => service.VoidAsync(march.Id));
    }
    ...
    public async Task Cannot_close_at_or_before_the_last_close()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() => service.CloseAsync(client, new DateOnly(2026, 3, 31), Guid.NewGuid()));
        await Assert.ThrowsAsync<InvalidOperationException>(() => service.CloseAsync(client, new DateOnly(2026, 2, 28), Guid.NewGuid()));

        await service.CloseAsync(client, new DateOnly(2026, 4, 30), Guid.NewGuid()); // a later period — fine
    }
}
